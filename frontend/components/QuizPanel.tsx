...
function scoreShortAnswer(userAnswer: string, expected: string): { score: number; label: string } {
  ...
  const stopWords = new Set([
    "the","a","an","is","are","was","were","it","in","of","to","and","or","for","that","this","with","by","on","at",
    "from","as","be","has","have","had","do","does","not","can","will","but","so","if","its","they","them","their",
    "which","what","when","where","how","all","each","no","any","into"
  ]);
...
interface Props {
  notebookId: number;
  /** activities => show only kinesthetic activities
   *  quiz => show only MCQ + Short Answer
   *  all => show everything (optional)
   */
  view?: "activities" | "quiz" | "all";
}
...
export default function QuizPanel({ notebookId, view = "all" }: Props) {
  const [plan, setPlan] = useState<QuizPlan | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // keep answers + activity progress in same component so they persist when switching tabs
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [completedActivities, setCompletedActivities] = useState<string[]>([]);
  const [scores, setScores] = useState<Scores | null>(null);
  const [grading, setGrading] = useState(false);

  const showActivities = view === "all" || view === "activities";
  ...
  const generate = async () => {
    ...
    setError(null);
    setScores(null);
    // NOTE: Don't wipe progress automatically on regenerate unless you want that.
    // If you DO want a full reset on regenerate, uncomment next 2 lines.
    setAnswers({});
    setCompletedActivities([]);
  ...
  const resetProgress = () => {
    setScores(null);
    if (showQuiz) setAnswers({});
    if (showActivities) setCompletedActivities([]);
    // If view === all, both reset anyway.
  };
  ...
  if (!plan && !loading) {
    return (
      <div style={{ padding: 24 }} role="region" aria-label="Practice quiz">
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-muted)", marginBottom: 8 }}>
          {view === "activities" ? "Kinesthetic Activities" : view === "quiz" ? "Practice Quiz" : "Practice Module"}
        </div>
        <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16, lineHeight: 1.5 }}>
          {view === "activities"
            ? "Generate kinesthetic activities from this material and track completion."
            : view === "quiz"
              ? "Generate interactive quiz questions from this material to test your understanding."
              : "Generate activities and quiz questions from this material."}
        </p>
        ...
        <button className="btn btn-primary" style={{ padding: "10px 24px" }} onClick={generate}>
          Generate {view === "activities" ? "Activities" : view === "quiz" ? "Quiz" : "Module"}
        </button>
      </div>
    );
  }
    ...
    <div style={{ padding: 24, overflowY: "auto", maxHeight: "100%" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
            {showActivities ? `${plan.activities?.length || 0} activities` : null}
            {showActivities && showQuiz ? " • " : null}
            {showQuiz ? `${plan.quiz.mcq.length} multiple choice • ${plan.quiz.short.length} short answer` : null}
          </div>
        </div>

        {/* Regenerate */}
        <button className="btn btn-ghost" style={{ padding: "8px 12px", fontSize: 12 }} onClick={generate} disabled={loading}>
          Regenerate
        </button>
      </div>

      {error && (
        <div style={{ color: "#EF4444", fontSize: 13, padding: "8px 12px", background: "#FEF2F2", borderRadius: 8, border: "1px solid #FECACA", marginBottom: 16 }}>
          {error}
        </div>
      )}

      {/* Activities Section (only when view allows) */}
      {showActivities && plan.activities && plan.activities.length > 0 && (
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--accent)", marginBottom: 16 }}>
            Kinesthetic Activities
          </div>
              ...
              <div key={act.id} className="card" style={{ marginBottom: 14, padding: 16 }}>
                <label style={{ display: "flex", alignItems: "flex-start", gap: 12, cursor: scores ? "default" : "pointer" }}>
                  <input
                    ...
                    onChange={(e) => {
                      if (scores) return;
                      if (e.target.checked) setCompletedActivities(prev => [...prev, act.id]);
                      else setCompletedActivities(prev => prev.filter(id => id !== act.id));
                    }}
                    disabled={!!scores}
                    style={{ marginTop: 4, width: 16, height: 16, accentColor: "var(--accent)" }}
                  />
      ...
      {!scores ? (
        <div style={{ display: "flex", gap: 10 }}>
          {/* Submit should only show in quiz view (or all) */}
          {showQuiz && (
            <button className="btn btn-primary" style={{ padding: "10px 24px" }} onClick={submit} disabled={grading}>
              {grading ? "Grading..." : "Submit Answers"}
            </button>
          ...
          <button className="btn btn-ghost" style={{ padding: "10px 16px", fontSize: 12 }} onClick={resetProgress}>
            Reset Progress
          </button>
        </div>
      ) : (
        <div className="card" style={{ padding: 20, background: "#FAFAFC" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
            {[
              ...
              <div key={label} style={{ padding: "10px 14px", background: "white", borderRadius: 8, border: "1px solid var(--border)" }}>
                <div style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</div>
                <div style={{ fontSize: 22, fontWeight: 800, color: val >= 70 ? "#34D399" : val >= 40 ? "#F59E0B" : "#EF4444" }}>
                  {val}%
                </div>
              </div>
            ))}
          ...
          <button className="btn btn-primary" style={{ padding: "10px 24px" }} onClick={() => { setScores(null); }}>
            Back
          </button>
        </div>
      )}
