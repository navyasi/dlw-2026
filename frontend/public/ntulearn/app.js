...
function _toHHMM(totalMinutes) {
    const h = String(Math.floor(totalMinutes / 60)).padStart(2, '0');
    const m = String(totalMinutes % 60).padStart(2, '0');
    return `${h}:${m}`;
}

function _conceptLabel(id) {
    return id.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
}

async function fetchDashboardData() {
    try {
        const res = await fetch(`${BACKEND_URL}/integrated-weekly`, {
            method: 'POST',
        ...
        console.warn('[NorthStar] Backend unavailable, using demo data:', err.message);
    }
}

function applyDashboardData(data) {
    const { weekly_report, study_plan, todays_schedule } = data;
    const scoreModel = weekly_report.predicted_score_model;

    const baseScore     = Math.round(scoreModel.base_expected);
    const currentPace   = Math.round(scoreModel.simulation_current_pace.expected_score);
    const predictedScore = Math.round(scoreModel.simulation_recommended_plan.expected_score);
    const scoreDiff = predictedScore - currentPace;
    ...
    const readinessTrend = document.getElementById('ns-stat-readiness-trend');
    if (readinessTrend) {
        readinessTrend.textContent = scoreDiff > 0
            ? `↑ +${scoreDiff} pts with recommended plan`
            : 'On track';
        readinessTrend.className = `ns-stat-trend${scoreDiff > 0 ? ' positive' : ''}`;
    }

    const predictedEl = document.getElementById('ns-stat-predicted');
    if (predictedEl) predictedEl.innerHTML = `${predictedScore}<span class="ns-stat-sub">%</span>`;

    const predictedTrend = document.getElementById('ns-stat-predicted-trend');
    if (predictedTrend) predictedTrend.textContent = `Projected: ${currentPace} → ${predictedScore}`;

    // Focus time = total plan minutes this week
    if (study_plan && study_plan.days) {
        let totalWeekMins = 0;
        study_plan.days.forEach(d => { totalWeekMins += d.total_study_minutes; });
        const focusEl = document.getElementById('ns-stat-focus');
        if (focusEl && totalWeekMins > 0) {
            const h = Math.floor(totalWeekMins / 60);
            const m = totalWeekMins % 60;
            focusEl.textContent = m > 0 ? `${h}h ${m}m` : `${h}h`;
        }
    }

    // ── Quick Insights ────────────────────────────────────────────────────────
    const recs = weekly_report.prescriptive_analysis.recommendations.slice(0, 3);
    ...
    // ── Today's Timeline ──────────────────────────────────────────────────────
    if (todays_schedule && todays_schedule.blocks && todays_schedule.blocks.length > 0) {
        const studyBlocks = todays_schedule.blocks.filter(b => b.block_type !== 'BREAK');
        ...
        studyBlocks.forEach((block, idx) => {
            ...
            const planId  = `p${idx + 1}`;
            const subjId  = `subj${idx + 1}`;

            const label = block.concept_ids.length > 0
                ? block.concept_ids.map(_conceptLabel).join(', ')
                : _conceptLabel(block.block_type);
            const typeLabel = _conceptLabel(block.block_type);

            state.todayPlan.push({
                id: planId,
                timeStart: _toHHMM(startMins),
                timeEnd:   _toHHMM(endMins),
                subjectTitle: `${typeLabel}: ${label}`,
                status: idx === 0 ? 'active' : 'pending',
                subjectId: subjId
            });

            state.subjects[subjId] = {
                id: subjId,
                title: label,
            ...
            startMins = endMins + 5; // 5-min gap between blocks
        });

        state.todayProgressPct = 0;
        state.selectedSubjectId = state.todayPlan[0] ? state.todayPlan[0].subjectId : 'subj1';
    }

    // ── Course Mastery Rings (use top-concept mastery instead of random) ───────
    state.conceptMasteries = weekly_report.priority_ranking.top_concepts;

    // ── Calendar Blocks from study plan ───────────────────────────────────────
    if (study_plan && study_plan.days) {
        state.calendarBlocks = [];
        study_plan.days.forEach(day => {
            // Convert JS getDay() (Sun=0) → our Mon=0 calendar index
            const jsDay  = new Date(day.date + 'T12:00:00').getDay();
            const calDay = (jsDay + 6) % 7;
            day.blocks.filter(b => b.block_type !== 'BREAK').forEach((block, idx) => {
                const label = block.concept_ids.length > 0
                    ? block.concept_ids.map(_conceptLabel).join(', ')
                    : _conceptLabel(block.block_type);
                const typeLabel = _conceptLabel(block.block_type);
                state.calendarBlocks.push({
                    id: `cal_${day.date}_${idx}`,
                    title: `${typeLabel}: ${label}`,
                    day: calDay,
                    type: block.block_type === 'HARD' ? 'focus' : 'task'
                });
            });
        });
    }

    // Re-render all widgets with real data
    renderStudySidebar();
    renderNsAppCourses();
    renderCalendar();
}
        ...
        card.innerHTML = `
            ...
            <div class="card-content">
                ...
                <div class="course-links">
                    <a href="http://localhost:3000/?platform=1" class="open-link">Open</a>
...
function renderNsAppCourses() {
    ...
    grid.innerHTML = displayCourses.map((c, idx) => {
        const conceptData = masteries[idx];
        // Use real mastery from backend if available, otherwise random placeholder
        const mastery = conceptData
            ? Math.round(conceptData.mastery * 100)
            : Math.floor(Math.random() * 40) + 40;
        const weakLabel = conceptData ? conceptData.concept_name : 'Arrays, Pointers';
        ...
        <div class="ns-card ns-course-card">
            ...
            <div class="ns-course-progress">
                ...
                <div class="ns-weak-topics">Requires Attention: ${weakLabel}</div>
            </div>
        </div>
        `;
    }).join('');
